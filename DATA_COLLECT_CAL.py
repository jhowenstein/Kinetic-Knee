...
Script collects data from knee sleeve with two inertial measurement units
	with only using the static calibration routine.
'''
	...
	if ser.inWaiting():
		...
		if cal_row.size == LINE_LENGTH:
			if first_line == 1:
				...
				CAL = np.vstack((CAL,cal_row))
			nCal += 1


ser.write(b'0')

cal_coeff = np.mean(CAL, axis=0)
...
np.savetxt(cal_filename, CAL, delimiter=',')
